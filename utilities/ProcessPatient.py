...
from utilities.GeminiFill import fill_map, fill_static_map
...
async def process_patient(patient: str, pa_path: str, referral_path: str):
    ...
    async def step_three_static(gemini_dict: dict):
        prompt = prompt_generator("static_pa_context_prompt", {"map_json": gemini_dict})
        result_gemini = await format_map(prompt=prompt, pdf_file=pa_path)
        print("....Static PA context added....")
        return result_gemini
    ...
    async def step_four_static(gemini_dict: dict, mistral_string: str):
        prompt = prompt_generator(
            "fill_static_map",
            {"extracted_referral": mistral_string, "map_json": gemini_dict},
        )
        result_gemini = await fill_static_map(prompt=prompt)
    ...
    if len(extracted_map) > 0:
        ...
        for result in results:
            if type(result) == list:
                ...
                res_dict["mistral"] = result
        res_dict["gemini"] = await asyncio.gather(step_three_static(res_dict["gemini"]))
        final_result = await asyncio.gather(
            step_four_static(
                gemini_dict=res_dict["gemini"], mistral_string=res_dict["mistral"]
            )
